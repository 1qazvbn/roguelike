...
const pauseMenu=new PauseMenu(pauseEl);pauseMenu.bindNewRun(()=>startNewRun());
...
function startNewRun(seed){if(!seed)seed=Math.random().toString(36).slice(2);seedStr=seed;lsSet('lastSeed',seed);if(loop)loop.stop();entities=[];level=null;player=null;depth=1;fps=60;fpsAccum=0;fpsCount=0;lastFps=0;rng=new RNG(hashSeed(seed));initLevel();input.keys.clear();menu.classList.add('hidden');pauseMenu.hide();state='play';loop=new Loop(update,render);loop.start();}
function initLevel(){
